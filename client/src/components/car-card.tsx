...
export function CarCard({ car, index }: CarCardProps) {
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className="group relative bg-white/95 backdrop-blur-sm rounded-3xl overflow-hidden border border-gray-200/50 shadow-xl shadow-black/5 hover:shadow-2xl hover:shadow-black/10 transition-all duration-500 hover:-translate-y-2"
      data-testid={`card-car-${car.id}`}
    >
      {/* Black glow on hover */}
      <div className="absolute -inset-0.5 bg-black rounded-3xl opacity-0 group-hover:opacity-10 blur transition duration-500" />
      ...
      <div className="p-8 bg-gradient-to-b from-white to-gray-50/50">
        <div className="flex justify-between items-start mb-6">
          ...
          <div className="text-right bg-black text-white px-5 py-2.5 rounded-2xl shadow-lg shadow-black/30">
            <p className="text-lg font-black" data-testid={`text-price-${car.id}`}>{car.price}</p>
            {car.mileage && (
              <p className="text-xs text-gray-300 mt-0.5 font-semibold" data-testid={`text-mileage-${car.id}`}>{car.mileage}</p>
            )}
          </div>
        </div>
        ...
        {(car.dealerName || car.dealerPhone || car.dealerAddress) && (
          <div className="mb-6 p-5 bg-gradient-to-br from-gray-900 via-black to-gray-900 text-white rounded-2xl border border-gray-700/30 shadow-xl shadow-black/10" data-testid={`dealer-info-${car.id}`}>
        ...
        <a 
          ...
          className="w-full py-4 bg-black hover:bg-gray-900 text-white rounded-2xl transition-all duration-300 font-black text-sm flex items-center justify-center gap-2 group/btn shadow-xl shadow-black/30 hover:shadow-2xl hover:shadow-black/40" 
